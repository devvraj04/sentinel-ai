/**
 * src/components/CreditOfficer/CustomerDetail.tsx
 * 
 * Full customer profile with:
 *  - Pulse Score gauge + risk tier
 *  - SHAP factor contributions (which signals drive the score)
 *  - Transaction history table
 *  - Score impact per transaction (how each txn changed the pulse score)
 *  - Pulse Score trend chart
 */
  ...
  return (
    <div className="flex flex-col items-center">
      <svg viewBox="0 0 200 120" width="200" height="120">
        {/* Background arc */}
        <path d="M 20 110 A 80 80 0 1 1 180 110" fill="none" stroke="#374151" strokeWidth="16" strokeLinecap="round" />
        {/* Green zone */}
        <path d="M 20 110 A 80 80 0 0 1 60 38" fill="none" stroke="#22c55e" strokeWidth="16" strokeLinecap="round" />
        {/* Yellow zone */}
        ...
        {/* Red zone */}
        <path d="M 140 38 A 80 80 0 0 1 180 110" fill="none" stroke="#ef4444" strokeWidth="16" strokeLinecap="round" />
        {/* Needle */}
        <line
          x1="100" y1="110"
        ...
        {/* Score label */}
        <text x="100" y="105" textAnchor="middle" fill={color} fontSize="22" fontWeight="bold">{score}</text>
      </svg>
...
export default function CustomerDetail({ customerId, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    if (activeTab === "score-impact") {
      api.get(`/customers/${customerId}/score-impact`, { params: { limit: 50 } })
        .then(r => setImpact(r.data.transactions));
    }
    if (activeTab === "history") {
      api.get(`/customers/${customerId}/pulse-history`, { params: { limit: 30 } })
  ...
  if (!profile) {
    return (
      <div className="text-center text-red-400 p-8">
        Customer not found.
        <button onClick={onBack} className="ml-4 text-blue-400 underline">Back</button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 p-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="ml-auto flex items-center gap-3">
          <span
            ...
            style={{ background: `${tierColor}22`, color: tierColor, border: `1px solid ${tierColor}` }}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 flex flex-col items-center">
          ...
          <p className="text-xs text-gray-500 mt-2">
            PD: {(profile.pd_probability * 100).toFixed(1)}% ·
      ...
      <div className="flex gap-2 mb-4 border-b border-gray-800 pb-2">
        {(["overview", "transactions", "score-impact", "history"] as Tab[]).map(tab => (
          ...
          >
            {tab === "overview"      ? "📊 SHAP Overview"
             : tab === "transactions" ? "📋 All Transactions"
             : tab === "score-impact" ? "⚡ Score Impact"
             : "📈 Score History"}
          </button>
        ))}
      </div>
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart
                  data={scoreData.top_factors.map(f => ({
                    name: f.human_readable.length > 30
                  ...
                  <tbody>
                    {scoreData.top_factors.map((f, i) => (
                      <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                        ...
                        <td className="py-2 text-right font-mono">{(f.contribution * 100).toFixed(2)}%</td>
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
              ...
              <span className="px-3 py-1 text-sm text-gray-400">
                {txnPage * TXN_PAGE_SIZE + 1}–{Math.min((txnPage + 1) * TXN_PAGE_SIZE, txnTotal)}
              </span>
              <button
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={[...impact].reverse()}>
                    ...
                    <Tooltip
                      ...
                      formatter={(val, name) => [val, "Pulse Score"]}
                    />
                    <ReferenceLine y={70} stroke="#ef4444" strokeDasharray="4 4" label={{ value: "Critical", fill: "#ef4444", fontSize: 10 }} />
                    <ReferenceLine y={45} stroke="#f97316" strokeDasharray="4 4" label={{ value: "At Risk", fill: "#f97316", fontSize: 10 }} />
                    <ReferenceLine y={25} stroke="#eab308" strokeDasharray="4 4" label={{ value: "Watch", fill: "#eab308", fontSize: 10 }} />
                        ...
                        <tr key={i} className={`border-b border-gray-800/40 hover:bg-gray-800/30 ${
                          ...
                          <td className="py-2 pr-4 text-center">
                            <span className={`font-bold text-base ${
                              t.score_impact > 0 ? "text-red-400"
                              : t.score_impact < 0 ? "text-green-400"
                              : "text-gray-500"
                            }`}>
                              {t.score_impact > 0 ? `+${t.score_impact}` : t.score_impact || "—"}
                            </span>
                          </td>
                          <td className="py-2 pr-4 text-center">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <LineChart data={[...history].reverse()}>
                  ...
                  <Tooltip
                    contentStyle={{ background: "#1f2937", border: "1px solid #374151" }}
                    labelFormatter={v => fmtDate(v)}
                    formatter={(val, name) => [val, name === "pulse_score" ? "Pulse Score" : "PD %"]}
                  />
                  <ReferenceLine y={70} stroke="#ef4444" strokeDasharray="3 3" />
                  <ReferenceLine y={45} stroke="#f97316" strokeDasharray="3 3" />
                  <ReferenceLine y={25} stroke="#eab308" strokeDasharray="3 3" />
                  <Line dataKey="pulse_score" stroke="#3b82f6" strokeWidth={2} dot={{ r: 3, fill: "#3b82f6" }} />
                </LineChart>
              </ResponsiveContainer>
              <div className="mt-4 overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {history.map((h, i) => (
                      <tr key={i} className="border-b border-gray-800/40 hover:bg-gray-800/30">
                        <td className="py-2 pr-4 text-gray-400 text-xs">{fmtDate(h.scored_at)}</td>
                        <td className="py-2 pr-4 text-center">
                          <span className="font-bold text-white">{h.pulse_score}</span>
                        </td>
                        <td className="py-2 pr-4">
                          <span className="text-xs font-semibold px-2 py-0.5 rounded"
                            style={{ color: TIER_COLORS[h.risk_tier], background: `${TIER_COLORS[h.risk_tier]}22` }}>
